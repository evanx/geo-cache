...
```
where for the request to `googleapis.com` where use the `authQuery` which includes the `key` sent in the request. However, we ensure we do not store or log `authQuery` but rather `query` from which the `key` has been deleted, as shown further above.
