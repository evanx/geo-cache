...
module.exports = async appx => {
    ...
    api.get('/stats', async ctx => {
        ...
        const [reqCount, setCount] = await multiExecAsync(client, multi => {
            multi.hgetall([config.redisNamespace, 'req:path:count:h'].join(':'));
            multi.hgetall([config.redisNamespace, 'set:path:count:h'].join(':'));
        });
        const stats = {reqCount, setCount};
    ...
    api.get('/maps/api/*', async ctx => {
        ...
        const [cachedContent] = await multiExecAsync(client, multi => {
            ...
            multi.hincrby([config.redisNamespace, 'req:path:count:h'].join(':'), path, 1);
        });
        if (cachedContent) {
